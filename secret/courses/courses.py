...
from django.views.decorators.csrf import csrf_exempt, csrf_protect
from secret.models import AddStudent, Class_Students, Courses, CourseLevels, TimePeriods, Course_Classes, Weekdays, classs
from django.conf import settings
...
def addstudent(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            ...
            users=classs.objects.filter(name=addcourses).first()
       
            if users:
                objec.students.add(users) #many to many
            else:
                cls = classs.objects.create(name=addcourses)
                objec.students.add(cls)
